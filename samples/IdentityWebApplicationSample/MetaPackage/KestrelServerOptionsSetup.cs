            ...
            options.Listen(address, port, listenOptions =>
            {
                ...
                if (certificateConfig.Exists())
                {
                    try
                    {
                        ...
                        {
                            if (certificateConfig.Value == "localhost" && _hostingEnvironment.IsDevelopment())
                            {
                                var storeLoader = new CertificateStoreLoader();
                                try
                                {
                                    certificate = storeLoader.Load("CN=localhost", "My", StoreLocation.CurrentUser, validOnly: false) ??
                                        storeLoader.Load("CN=localhost", "My", StoreLocation.LocalMachine, validOnly: false);
                                }
                                catch (Exception)
                                {
                                }
