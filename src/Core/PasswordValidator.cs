...
namespace Microsoft.AspNetCore.Identity
{
    ...
    public class PasswordValidator<TUser> : IPasswordValidator<TUser> where TUser : class
    {
        ...
        /// </summary>
        /// <param name="manager">The <see cref="IUserManager{TUser}"/> to retrieve the <paramref name="user"/> properties from.</param>
        /// <param name="user">The user whose password should be validated.</param>
        /// <param name="password">The password supplied for validation</param>
        /// <returns>The task object representing the asynchronous operation.</returns>
        public virtual Task<IdentityResult> ValidateAsync(IUserManager<TUser> manager, TUser user, string password)
        {
            if (password == null)
            {
