...
namespace Microsoft.AspNetCore.Identity.Service.Mvc
{
    public class TokenRequestModelBinder : IModelBinder
    {
        public async Task BindModelAsync(ModelBindingContext bindingContext)
        {
            ...
            if (bindingContext.ModelType.Equals(typeof(TokenRequest)))
            {
                ...
                else
                {
                    ...
                    var factory = httpContext.RequestServices.GetRequiredService<ITokenRequestFactory>();
                    bindingContext.Result = ModelBindingResult.Success(await factory.CreateTokenRequestAsync(requestParameters));
                }
            }
        }
    }
